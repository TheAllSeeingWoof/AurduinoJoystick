...
/* IRQ number. */
static int irq_gpio11 = -1;
static int irq_gpio08 = -1;
...
/* interrupt handler called when rising edge on SCLK (GPIO_11) occurs;
   read the level from MOSI (GPIO_10) */
   
   //int broj;
   
static irqreturn_t h_irq_gpio11(int irq, void *data)
{
    static char value = -1;

	printk("Interrupt from IRQ11 0x%x\n", irq);
	printk("gpio_ss = %d", gpio_ss);
	if(gpio_ss == 0){
		value = GetGpioPinValue(GPIO_10);
		printk("GPIO_10_PRE valueX = %x\n", value);
		received_data = (received_data  << 1) | value;
		printk("GPIO_10_PRE received_dataS = %x\n", received_data);	
...
/* interrupt handler called when rising edge on SS (GPIO_08) occurs;*/
static irqreturn_t h_irq_gpio08(int irq, void *data)
{
    static char value = -1;

    printk("Interrupt from IRQ08 0x%x\n", irq);

    value = GetGpioPinValue(GPIO_08);
	...
	if(value != 0){
		...
		received_data = 0;
		printk("Output dataX = %x", output_data);
		...
		{
			printk("Button B - DOWN");
		}
		else if(output_data == 4)
		{
			printk("Button C - LEFT");
		}
		else if(output_data == 8)
		{
			printk("Button D - RIGHT");
		}
	}else{
		gpio_ss = 0;
		printk("SS = 0");
...
int gpio_driver_init(void)
{
    ...
    /* SPI PINS */
	SetInternalPullUpDown(GPIO_10, PULL_UP);			//MOSI PULL UP
    	SetGpioPinDirection(GPIO_10, GPIO_DIRECTION_IN);		//MOSI
	
	
	SetInternalPullUpDown(GPIO_11, PULL_UP);			//SLCK PULL UP
	SetGpioPinDirection(GPIO_11, GPIO_DIRECTION_IN);		//SCLK
	
	SetInternalPullUpDown(GPIO_08, PULL_UP);			//SS PULL UP
	SetGpioPinDirection(GPIO_08, GPIO_DIRECTION_IN);		//SS
	
    /* Initialize gpio 11 ISR. - CLOCK */
    result = gpio_request_one(GPIO_11, GPIOF_IN, "irq_gpio11");
	if(result != 0)
    {
        printk("Error: GPIO request failed!\n");
        goto fail_irq;
    }
    irq_gpio11 = gpio_to_irq(GPIO_11);
	result = request_irq(irq_gpio11, h_irq_gpio11,
      IRQF_TRIGGER_RISING, "irq_gpio11", (void *)(h_irq_gpio11));
	if(result != 0)
	{
	...
	/* Initialize gpio 08 ISR. - SS*/
    result = gpio_request_one(GPIO_08, GPIOF_IN, "irq_gpio08");
	if(result != 0)
    {
        printk("Error: GPIO request failed!\n");
        goto fail_irq;
    }
    irq_gpio08 = gpio_to_irq(GPIO_08);
	result = request_irq(irq_gpio08, h_irq_gpio08,
      IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "irq_gpio08", (void *)(h_irq_gpio08));
	if(result != 0)
	{
        printk("Error: ISR not registered!\n");
        goto fail_irq;
...
void gpio_driver_exit(void)
{
    ...
    disable_irq(irq_gpio11);
    free_irq(irq_gpio11, h_irq_gpio11);
    gpio_free(GPIO_11);
	
	disable_irq(irq_gpio08);
	free_irq(irq_gpio08, h_irq_gpio08);
	gpio_free(GPIO_08);
    ...
    SetInternalPullUpDown(GPIO_11, PULL_NONE);
	SetInternalPullUpDown(GPIO_10, PULL_NONE);
	SetInternalPullUpDown(GPIO_08, PULL_NONE);
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos)
{
	...
	switch(output_data){
		case 0:
			strcpy(gpio_driver_buffer, "NONE");
			break;
		case 1:
			strcpy(gpio_driver_buffer, "UP");
			break;
		case 2:
			strcpy(gpio_driver_buffer, "DOWN");
			break;
		case 4:
			strcpy(gpio_driver_buffer, "LEFT");
			break;
		case 8:
			strcpy(gpio_driver_buffer, "RIGHT");
			break;
	}
